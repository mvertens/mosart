...
!-----------------------------------------------------------------------
   ...
   subroutine mosart_histflds_init()
      ...
      ! capability, so it needs to throw an error when h0i is empty. For this
      ! reason MOSART always need at least one instantaneous field so that
      ! h0i will not be empty.
      call mosart_hist_addfld (fname='STORAGE_MCH', units='m3',  &
